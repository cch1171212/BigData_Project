        ...
        if request.method == "GET":
            ...
            if memid:
                if content != "":
                    ...
                    dto.save()
                    logger.info(member.gender+","+member.birthday[0:2]+","+post.num+",C")
...
def Like_set(request):
    ...
    if likes.filter(user_id = memid):
        ...
        if  str(post.num) not in str(like.num):
            ...
            like.save()
            logger.info(member.gender+","+member.birthday[0:2]+","+post.num+",L")
    else:
        dto =Like(
        num = post.num,
        user_id = member,
...
def playlist_set( request ) :
    ...
    if plays.filter(user_id = memid):
        ...
        if  str(post.num) not in str(play.num):
            ...
            play.save()
            logger.info(member.gender+","+member.birthday[0:2]+","+post.num+",P")
    else:
        dto = PlayList(
            num = post.num,
            user_id = member,
